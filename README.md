...
패키지 버전은 [Semantic Versioning (SemVer)](https://semver.org/) 규칙을 따릅니다:

**버전 형식: `MAJOR.MINOR.PATCH` (예: 1.2.3)**

- **MAJOR (1.0.0)**: 하위 호환되지 않는 API 변경
  - 기존 컴포넌트의 props 제거 또는 변경
  - 컴포넌트 이름 변경 또는 제거
  - 테마 구조의 근본적인 변경
- **MINOR (0.1.0)**: 하위 호환되는 기능 추가
  - 새로운 컴포넌트 추가
  - 기존 컴포넌트에 새로운 props 추가
  - 새로운 테마 또는 토큰 추가
...
# 예: "0.1.0" -> "0.1.1" (버그 수정)
# 예: "0.1.0" -> "0.2.0" (새 기능 추가)
# 예: "0.1.0" -> "1.0.0" (Breaking Changes)
```

**⚠️ 중요사항:**

- npm 레지스트리에 배포된 버전은 **절대 덮어쓸 수 없습니다**
- 같은 버전으로 재배포 시도 시 `400 Bad Request` 에러가 발생합니다
- 배포 전 반드시 버전을 올려야 합니다
- 실수로 잘못된 버전을 배포한 경우, 새로운 버전으로 수정사항을 배포해야 합니다
